...
def render_copy_into_sql(
    target_table: str, parquet_path: str, catalog: str = DEFAULT_CATALOG, schema: str = DEFAULT_SCHEMA
) -> str:
    qualified = plain_qualified_table_name(target_table, catalog, schema)
    escaped_path = str(parquet_path).replace("'", "''")
    return f"COPY INTO {qualified}\n" f"FROM '{escaped_path}'\n" "FILEFORMAT = PARQUET;\n"
...
def render_load_sql(
    create_table_sql: str, copy_into_sql: str, pyspark_snippet: str, local_path_warning: str = ""
) -> str:
...
def generate_databricks_load_artifacts(
    ...
    if approved_mapping_path and os.path.exists(approved_mapping_path):
        try:
            ...
            mapping_rows = [
                row for row in all_rows if str(row.get("target_table") or "").strip().lower() == target_table.lower()
            ]
        except Exception:
            mapping_rows = []
    ...
    if is_local_parquet_path(parquet_path):
        local_path_warning = (
            "The Parquet path is local. Databricks must be able to access this path, or use a cloud path override."
        )
