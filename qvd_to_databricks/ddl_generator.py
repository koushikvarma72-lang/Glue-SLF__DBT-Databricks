...
def validate_mapping_rows_for_ddl(mapping_rows: list[dict]) -> list[dict]:
    ...
    for index, row in enumerate(mapping_rows):
        ...
        if target_type not in ALLOWED_TARGET_TYPES:
            errors.append(
                {"row": index, "field": "target_type", "error": f"Unsupported target_type: {target_type or '(empty)'}."}
            )
...
def render_create_table_sql(target_table: str, rows: list[dict], catalog_schema: str = "main.qvd_raw") -> str:
    columns = [
        (str(row.get("target_column") or "").strip(), str(row.get("target_type") or "").strip().upper()) for row in rows
    ]
    columns.extend(AUDIT_COLUMNS)

    column_lines = [f"  {quote_identifier(column_name)} {target_type}" for column_name, target_type in columns]
...
def generate_delta_ddl_from_approved_mapping(
    mapping_csv_path: str, ddl_dir: str, catalog_schema: str = "main.qvd_raw"
) -> dict:
