...
def _target_type_for_field(field: dict, category: str) -> tuple[str, str, float, str]:
    ...
    if category == "NUMERIC_LIKE":
        ...
        if any(
            token in name
            for token in ("amount", "sales", "price", "cost", "value", "revenue", "margin", "difference", "ops")
        ):
            ...
            errors.append(
                {
                    "row": index,
                    "field": "review_status",
                    "error": f"Unsupported review_status: {review_status or '(empty)'}.",
                }
            )
            ...
            errors.append(
                {
                    "row": index,
                    "field": "review_status",
                    "error": "All rows must be AUTO_APPROVED or MANUALLY_APPROVED before saving.",
                }
            )
...
def write_approved_mapping_artifacts(session_id: str, output_dir: str, mapping_rows: list[dict]) -> dict:
    ...
    payload = {
        ...
        "approved_count": sum(
            1 for row in normalized_rows if row.get("review_status") in {"AUTO_APPROVED", "MANUALLY_APPROVED"}
        ),
    }
