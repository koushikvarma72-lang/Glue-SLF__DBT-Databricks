...
def _load_execution_create_sql(
    output_dir: str, target_table: str, config: DatabricksConnectionConfig, fallback_sql_path: str
) -> str:
...
def render_execution_copy_into_sql(target_table: str, parquet_path: str, config: DatabricksConnectionConfig) -> str:
    ...
    return f"COPY INTO {_qualified_table(config, target_table)}\n" f"FROM '{escaped_path}'\n" "FILEFORMAT = PARQUET;\n"
...
def _list_result_has_parquet(result: dict) -> bool:
    ...
    for row in rows:
        ...
        if any(
            str(value or "").lower().endswith(".parquet") or ".parquet" in str(value or "").lower() for value in values
        ):
            return True
    return False
...
def precheck_execution(
    output_dir: str, target_table: str, mode: str, config: DatabricksConnectionConfig | None = None
) -> dict:
    errors = []
    warnings = []
    ...
    for path, label in (
        (create_sql_path, "create_table.sql"),
        (load_sql_path, "load_parquet_to_delta.sql"),
        (load_config_path, "load_config.json"),
    ):
    ...
    if (
        mode in {"execute_ddl_load", "full_migration"}
        and databricks_readable_path.startswith("/Volumes")
        and not os.path.exists(upload_status_path)
    ):
        warnings.append(
            "No volume upload status artifact found. Upload Parquet to the selected volume before executing load."
        )
...
def write_upload_status(output_dir: str, result: dict) -> dict:
    ...
    _write_json(path, {**result, "created_at": datetime.now(timezone.utc).isoformat()})
...
def execute_qvd_migration(
    ...
    validation = (
        _read_json(precheck["paths"]["validation_report"])
        if os.path.exists(precheck["paths"]["validation_report"])
        else {}
    )
    ...
    if not errors:
        ...
        parquet_path = precheck.get("databricks_readable_path") or _target_parquet_path(
            config, target_table, load_config.get("parquet_path") or ""
        )
        ...
        schema_sql = (
            f"CREATE SCHEMA IF NOT EXISTS {_quote_identifier(config.catalog)}.{_quote_identifier(config.schema)}"
        )
        ...
        if mode == "generate_sql_only":
            ...
            try:
                ...
                if mode in {"execute_ddl_load", "full_migration"}:
                    ...
                    insert_result = executor.execute_statement(
                        insert_cast_sql, log_callback=logs.append, stage="insert_cast"
                    )
                    ...
                    if loaded_row_count is not None and source_row_count is not None:
                        ...
                        if not match:
                            ...
                            warnings.append(
                                f"Loaded row count {loaded_row_count} does not match source row count {source_row_count}."
                            )
            except Exception as exc:
                execution_status = "failed"
    ...
    report = {
        ...
        "row_count_match": bool(loaded_row_count == source_row_count)
        if loaded_row_count is not None and source_row_count is not None
        else None,
