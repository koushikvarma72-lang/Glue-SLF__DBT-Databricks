...
def _match_status(detected_runtime_type: str, approved_target_type: str) -> tuple[str, str]:
    ...
    if detected_runtime_type == "QLIK_DATE_SERIAL":
        return (
            ("MATCH", "")
            if approved == "DATE"
            else ("MISMATCH", "Qlik date serial values do not match approved target type.")
        )
    if detected_runtime_type == "BOOLEAN_LIKE":
        return (
            ("MATCH", "")
            if approved == "BOOLEAN"
            else ("NEEDS_REVIEW", "Boolean-like values may need target type review.")
        )
    if detected_runtime_type == "INTEGER":
        return (
            ("MATCH", "")
            if approved in {"INT", "BIGINT"}
            else ("MISMATCH", "Integer runtime values do not match approved target type.")
        )
    if detected_runtime_type == "DECIMAL":
        return (
            ("MATCH", "")
            if approved in {"DOUBLE", "DECIMAL(18,2)"}
            else ("MISMATCH", "Decimal runtime values do not match approved target type.")
        )
    if detected_runtime_type == "STRING":
        return (
            ("MATCH", "")
            if approved == "STRING"
            else ("MISMATCH", "String runtime values do not match approved target type.")
        )
...
def profile_qvd_columns(
    file_path: str, approved_mapping_rows: list[dict] | None = None, limit: int | None = None
) -> dict:
