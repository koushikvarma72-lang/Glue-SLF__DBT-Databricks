...
def _require_pandas():
    try:
        ...
        raise RuntimeError(
            "Pandas is required for QVD to Parquet conversion. Install it with `pip install pandas`."
        ) from exc
...
def convert_qvd_to_parquet(
    file_path: str, approved_mapping_path: str, output_dir: str, batch_id: str | None = None
) -> dict:
    warnings: list[str] = []
    errors: list[str] = []
    ...
    try:
        ...
        target_tables = sorted(
            {
                ...
                if str(row.get("target_table") or "").strip()
            }
        )
        ...
        for row in mapping_rows:
            ...
            if source_column not in source_df.columns:
                warnings.append(
                    f"Source column '{source_column}' not found in QVD rows; output column '{target_column}' will be null."
                )
        ...
        target_df["_record_hash"] = [
            _record_hash(record, data_columns) for record in target_df[data_columns].to_dict(orient="records")
        ]
