...
class DatabricksConnectionConfig:
    ...
    def from_payload(cls, payload: dict) -> DatabricksConnectionConfig:
        return cls(
            ...
            personal_access_token=str(
                payload.get("personal_access_token") or payload.get("personalAccessToken") or payload.get("token") or ""
            ).strip(),
...
def validate_config(
    config: DatabricksConnectionConfig, require_token: bool = True, require_warehouse: bool = True
) -> list[str]:
    errors = []
    ...
    # SQL Warehouse + Unity Catalog catalog/schema are only needed for the
    # warehouse-based (dbt/SQL) flow. The PySpark flow runs on a cluster, so
    # these are not required just to verify the connection.
...
class DatabricksApiClient:
    ...
    def patch(self, path: str, payload: dict, timeout: int = 120) -> dict:
    ...
    def _request(
        ...
        body = (
            ...
            else (json.dumps(payload or {}).encode("utf-8") if payload is not None else None)
        )
        ...
        try:
            ...
            try:
                ...
                pass
            err = _structured_api_error(
                _friendly_databricks_error(method, path, exc.code, detail), method, path, "http_error", exc.code
            )
            if db_error_code:
            ...
            if isinstance(exc.reason, socket.timeout):
                return _structured_api_error(FRIENDLY_DATABRICKS_TIMEOUT, method, path, "timeout")
            return _structured_api_error(
                f"Databricks API {method} {path} failed: {exc.reason}", method, path, "url_error"
            )
        ...
        try:
            ...
            return _structured_api_error(
                f"Databricks API {method} {path} returned invalid JSON: {exc}", method, path, "json_decode_error"
            )


def _structured_api_error(
    message: str, method: str, path: str, error_type: str, status_code: int | None = None
) -> dict:
    payload = {
        "success": False,
        "status": "failed",
...
def _friendly_databricks_error(method: str, path: str, status_code: int, detail: str) -> str:
    ...
    if status_code in {401, 403} and ("credential was not sent" in lowered or "unsupported type" in lowered):
        reason = (
            "Databricks rejected the access token — it is invalid, expired, or lacks access. "
            "Generate a new Personal Access Token (User Settings → Developer → Access tokens) and paste it again."
        )
...
def test_databricks_connection(config: DatabricksConnectionConfig, client=None, require_warehouse: bool = True) -> dict:
    errors = validate_config(config, require_warehouse=require_warehouse)

    # Cluster/PySpark flow: no SQL warehouse — just verify the token + workspace
    # are valid by calling the current-user endpoint.
    if not require_warehouse:
        ...
        try:
            ...
            if me.get("success") is False:
                errors.extend(me.get("errors") or [me.get("error", "Databricks connection failed.")])
                return {"success": False, "checks": checks, "errors": errors, "config": config.masked()}
    ...
    try:
        ...
        if catalog_response.get("success") is False:
            errors.extend(
                catalog_response.get("errors") or [catalog_response.get("error", "Catalog validation failed.")]
            )
            return {"success": False, "checks": checks, "errors": errors, "config": config.masked()}
        checks["catalog"] = True
...
def list_catalogs(config: DatabricksConnectionConfig, client=None) -> list[dict]:
    # The Unity Catalog list API paginates; follow next_page_token so workspaces
    # with more catalogs than one page still return ALL of them (otherwise the
    # picker silently shows only the first page).
    client = client or DatabricksApiClient(config)
    out: list[dict] = []
    page_token = None
    for _ in range(100):  # bounded paging guard
        path = "/api/2.1/unity-catalog/catalogs?max_results=200"
        if page_token:
            path += f"&page_token={urllib.parse.quote(str(page_token), safe='')}"
        payload = client.get(path)
        if payload.get("success") is False:
            break
        for row in payload.get("catalogs") or []:
            out.append(
                {
                    "name": row.get("name", ""),
                    "type": row.get("catalog_type") or row.get("type", ""),
                    "owner": row.get("owner", ""),
                }
            )
...
def list_schemas(config: DatabricksConnectionConfig, catalog: str, client=None) -> list[dict]:
    ...
    out: list[dict] = []
    page_token = None
    base = f"/api/2.1/unity-catalog/schemas?catalog_name={urllib.parse.quote(str(catalog or ''), safe='')}&max_results=200"
    for _ in range(100):  # bounded paging guard
        path = base + (f"&page_token={urllib.parse.quote(str(page_token), safe='')}" if page_token else "")
        payload = client.get(path)
        if payload.get("success") is False:
            break
        for row in payload.get("schemas") or []:
            out.append(
                {
                    "name": row.get("name", ""),
