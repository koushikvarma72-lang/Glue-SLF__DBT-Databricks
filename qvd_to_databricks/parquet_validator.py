...
def _find_parquet_files(parquet_path: str) -> list[str]:
    ...
    if not os.path.isdir(parquet_path):
        return []
    return [os.path.join(parquet_path, name) for name in sorted(os.listdir(parquet_path)) if name.endswith(".parquet")]
...
def validate_parquet_output(
    ...
    inspection_json_path: str | None = None,
    ...
    try:
        ...
        checks.append(
            _check("approved_mapping_readable", False, {"approved_mapping_path": approved_mapping_path}, str(exc))
        )
        return _summary(False, checks, errors + [str(exc)], null_percentages)
    ...
    checks.append(
        _check("approved_target_columns_exist", not missing_target_columns, {"missing_columns": missing_target_columns})
    )
    ...
    for row in mapping_rows:
        ...
        if target_type == "DATE":
            invalid = [
                value for value in frame[target_column].dropna().head(20).tolist() if not _is_date_like_value(value)
            ]
            if invalid:
