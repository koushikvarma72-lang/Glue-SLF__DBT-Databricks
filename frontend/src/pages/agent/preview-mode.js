...
export function bindPreviewActions(sqlOutput) {
  const btn = document.getElementById('copy-agent-sql');
  btn?.addEventListener('click', async () => {
    try {
      ...
      notify('SQL copied to clipboard', { kind: 'success', title: 'Copied' });
    } catch (err) {
      btn.textContent = '⚠ Copy failed';
      setTimeout(() => { btn.textContent = 'Copy SQL'; }, 1500);
      notify('Copy failed — select and copy manually', { kind: 'error', title: 'Copy failed' });
    }
  });
}
