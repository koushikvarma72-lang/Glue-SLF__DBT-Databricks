...
function renderQvdMappingErrors(errors) {
  ...
  return `
    <div class="qvd-validation-errors" role="status" aria-live="polite">
      ${items.slice(0, 8).map(err => {
        const hasRow = err.row !== undefined && err.row !== null;
        const cell = hasRow && err.field ? `${Number(err.row)}:${err.field}` : '';
        return `
        <div${cell ? ` class="qvd-error-jump" data-qvd-error-cell="${escapeHtml(cell)}" role="button" tabindex="0" title="Jump to this cell" style="cursor:pointer"` : ''}>${hasRow ? `Row ${Number(err.row) + 1}: ` : ''}${escapeHtml(err.error || err)}</div>
      `;
      }).join('')}
      ...
      <table class="qvd-mapping-table" style="width:100%;border-collapse:collapse;font-size:12px;background:var(--bg-primary)">
        ...
        <tbody>
          ${(rows || []).map((row, index) => {
            ...
            const low = pct < 60;
            return `
            <tr data-qvd-map-row="${index}" class="${row.review_status === 'NEEDS_REVIEW' ? 'qvd-needs-review-row' : ''}">
              ...
              <td style="padding:8px;border-bottom:1px solid var(--border)">
                <input class="qvd-mapping-input" data-qvd-cell="${index}:target_table" data-qvd-map-index="${index}" data-qvd-map-field="target_table" value="${escapeHtml(row.target_table || '')}" />
              </td>
              <td style="padding:8px;border-bottom:1px solid var(--border)">
                <input class="qvd-mapping-input" data-qvd-cell="${index}:target_column" data-qvd-map-index="${index}" data-qvd-map-field="target_column" value="${escapeHtml(row.target_column || '')}" />
              </td>
              <td style="padding:8px;border-bottom:1px solid var(--border)">
                <select class="qvd-mapping-select" data-qvd-cell="${index}:target_type" data-qvd-map-index="${index}" data-qvd-map-field="target_type">
              ...
              <td style="padding:8px;border-bottom:1px solid var(--border)">
                <span style="font-weight:600;color:${low ? 'var(--warning,#d97706)' : 'var(--text-primary)'}">${pct}%</span>${low ? ' <span class="badge badge-warning">low</span>' : ''}
              </td>
              <td style="padding:8px;border-bottom:1px solid var(--border)">
                <select class="qvd-mapping-select" data-qvd-cell="${index}:review_status" data-qvd-map-index="${index}" data-qvd-map-field="review_status">
...
function renderQvdDdlResult(result) {
  ...
  return `
    <section class="qvd-ddl-result" role="status" aria-live="polite">
      ...
      <main class="qvd-review-main">
        ...
        <section class="qvd-review-card">
          ...
          <div class="qvd-business-shell">
            <nav class="qvd-business-section-nav" role="tablist" aria-label="Inspect Mapping sections">
              ${sections.map(([id, label, helper]) => `
                <button type="button" role="tab" aria-selected="${active === id ? 'true' : 'false'}" class="qvd-business-section-link ${active === id ? 'active' : ''}" data-qvd-mapping-section="${id}">
  ...
  if (active === 'ddl') {
    return `
      ${renderQvdSectionHeader('Databricks DDL', 'Generate and inspect Delta table DDL after approving the mapping.', `
        <button class="btn btn-secondary" id="qvd-generate-ddl-btn" ${!approved || state.isGeneratingQvdDdl ? 'disabled' : ''}${!approved ? ' title="Save the approved mapping first"' : ''}>
...
export function setupQvdMappingReview() {
  document.querySelectorAll('[data-qvd-mapping-section]').forEach(button => {
    button.addEventListener('click', () => {
      ...
      // Re-render destroys the clicked button; restore focus to the newly-active tab.
      document.querySelector(`[data-qvd-mapping-section="${activeQvdMappingSection}"]`)?.focus();
    });
  });
  document.querySelectorAll('[data-qvd-error-cell]').forEach(el => {
    const jump = () => focusQvdErrorCell(el.dataset.qvdErrorCell);
...
function focusQvdErrorCell(cellKey) {
  if (!cellKey) return;
  const cell = document.querySelector(`[data-qvd-cell="${cellKey}"]`);
  if (!cell) return;
  cell.scrollIntoView({ behavior: 'smooth', block: 'center' });
  cell.style.outline = '2px solid var(--danger,#dc2626)';
  cell.focus({ preventScroll: true });
  cell.addEventListener('input', () => { cell.style.outline = ''; }, { once: true });
}

function highlightQvdErrors(errors) {
  const first = (errors || []).find(e => e.row !== undefined && e.row !== null && e.field);
  if (first) focusQvdErrorCell(`${Number(first.row)}:${first.field}`);
}
...
async function handleQvdSaveApprovedMapping() {
  ...
  if (errors.length) {
    ...
    // store.set re-renders page-content synchronously; defer so we query the fresh DOM.
    requestAnimationFrame(() => highlightQvdErrors(errors));
    return;
  }
  ...
  try {
    ...
    notify(err.message || 'Approved mapping save failed', { kind: 'error', title: 'Save failed' });
  }
}
...
async function handleQvdGenerateDdl() {
  ...
  try {
    ...
    notify(err.message || 'QVD DDL generation failed', { kind: 'error', title: 'DDL generation failed' });
  }
}

async function handleQvdResetMapping() {
  if (!(await confirmModal('Discard all your edits and restore the suggested mapping? This cannot be undone.', {
    title: 'Reset to suggested mapping?', confirmLabel: 'Discard edits', danger: true,
  }))) return;
