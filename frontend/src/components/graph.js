...
export class GraphComponent {
  ...
  update(graphData) {
    ...
    // ── Hard safety cap ──────────────────────────────────────────────────────
    // A d3 force simulation (forceManyBody + forceCollide) plus the per-node SVG
    // it draws is O(n²)-ish; with thousands of tables/links it pegs the CPU and
    // balloons memory, freezing not just the tab but the whole machine. Above
    // these limits we skip the graph entirely and show a note — the Tables /
    // Columns / Relationships list views remain the way to explore large models.
    const MAX_NODES = 400;
    const MAX_EDGES = 900;
    if (this.nodes.length > MAX_NODES || this.edges.length > MAX_EDGES) {
      const n = this.nodes.length, e = this.edges.length;
      this.g.append('text')
        .attr('x', width / 2).attr('y', height / 2 - 10).attr('text-anchor', 'middle')
        .style('fill', 'var(--text-secondary)').style('font-size', '15px').style('font-weight', '600')
        .text('Model too large to render as a graph');
      this.g.append('text')
        .attr('x', width / 2).attr('y', height / 2 + 15).attr('text-anchor', 'middle')
        .style('fill', 'var(--text-muted)').style('font-size', '12px')
        .text(`${n.toLocaleString()} tables · ${e.toLocaleString()} links — use the list views to explore.`);
      this.nodes = [];
      this.edges = [];
      return;
    }
    ...
    nodeGs.each(function(d) {
      ...
      // Status icon (with a text label for assistive tech, not icon-only)
      const statusIcon = isMissing ? '❌' : isProcessing ? '⏳' : '✅';
      const statusWord = isMissing ? 'missing' : isProcessing ? 'processing' : 'ready';
      const statusText = g.append('text')
        .attr('x', -nodeWidth / 2 + 10)
        .attr('y', -h / 2 + 24)
        .attr('font-size', '11px')
        .attr('aria-label', `Status: ${statusWord}`)
        .text(statusIcon);
      statusText.append('title').text(`Status: ${statusWord}`);
    ...
    nodeGs.on('mouseover', (event, d) => {
      ...
      this.tooltip
        ...
        .html(`
          ...
          <div class="tooltip-meta">${escapeHtml(d.description || '')}</div>
          <div class="tooltip-meta" style="margin-top:4px">${(d.fields || []).length} fields · ${(d.rows || 0).toLocaleString()} rows · ${escapeHtml(d.type || '')}</div>
        `);
    })
    .on('mousemove', (event) => {
    ...
    if (this.selectedNodeId) this.highlightConnections(this.selectedNodeId);
  ...
  /**
   * Highlight a node and the edges/nodes directly connected to it, dimming
   * everything else. Pass `null` to clear the highlight.
   */
  highlightConnections(nodeId) {
    this.selectedNodeId = nodeId;
    if (!this.nodeGs || !this.edgePaths || !this.edgeLabels) return;

    const edgeSourceId = (e) => (typeof e.source === 'object' ? e.source.id : e.source);
    const edgeTargetId = (e) => (typeof e.target === 'object' ? e.target.id : e.target);
    const touchesSelection = (e) => edgeSourceId(e) === nodeId || edgeTargetId(e) === nodeId;

    if (!nodeId) {
      this.nodeGs.style('opacity', 1);
      this.edgePaths.style('opacity', 1).style('stroke', '#334155').style('stroke-width', 2);
      this.edgeLabels.style('opacity', 1);
      return;
    }

    const connected = new Set([nodeId]);
    this.edges.forEach(e => {
      if (edgeSourceId(e) === nodeId) connected.add(edgeTargetId(e));
      if (edgeTargetId(e) === nodeId) connected.add(edgeSourceId(e));
    });

    this.nodeGs.style('opacity', d => (connected.has(d.id) ? 1 : 0.2));
    this.edgePaths
      .style('opacity', d => (touchesSelection(d) ? 1 : 0.1))
      .style('stroke', d => (touchesSelection(d) ? '#2f7d5b' : '#334155'))
      .style('stroke-width', d => (touchesSelection(d) ? 3 : 2));
    this.edgeLabels.style('opacity', d => (touchesSelection(d) ? 1 : 0.1));
  }
